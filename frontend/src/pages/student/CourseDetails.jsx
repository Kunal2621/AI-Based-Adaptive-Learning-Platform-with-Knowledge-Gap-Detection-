...
export default function CourseDetails() {
  ...
  // Modal State for Text Notes / Lesson Content
    ...
    <div className="p-5 md:p-6 space-y-5">
        ...
        <div className="space-y-3">
          {!course.topics?.length ? (
            ...
            course.topics.map((topic, i) => (
              <div key={i} className="glass-card rounded-xl overflow-hidden border border-outline-variant/30">
                <button
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-semibold text-sm text-left">{topic.title || topic}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    {topic.materials?.length > 0 && (
                      <span className="text-xs text-on-surface-variant">{topic.materials.length} lessons</span>
                    )}
                    <span
                      ...
                      topic.materials.map((m, j) => {
                        const cfg =
                          {
                            ...
                            note: { icon: "description", color: "text-purple-600", bg: "bg-purple-50" },
                        ...
                        return (
                          <div
                            ...
                            className="flex items-center gap-3 p-3 bg-surface-container rounded-xl hover:bg-primary/10 transition-colors group cursor-pointer"
                          >
                            <div className={`w-8 h-8 rounded-lg ${cfg.bg} flex items-center justify-center flex-shrink-0`}>
                              <span
                                className={`material-symbols-outlined text-lg ${cfg.color}`}
                                style={{ fontVariationSettings: '"FILL" 1' }}
                              >
                                {cfg.icon}
                              </span>
                            </div>
                            <span className="text-sm flex-1 font-medium text-on-surface">{m.title}</span>
                            <span className="material-symbols-outlined text-outline text-sm group-hover:text-primary transition-colors">
                              {m.url ? "open_in_new" : "visibility"}
                            </span>
                          </div>
                        );
                      })
      ...
      {activeMaterial && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full p-6 space-y-4 shadow-2xl relative max-h-[80vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b pb-3">
              <h3 className="font-bold text-base text-gray-900">{activeMaterial.title}</h3>
              <button
            ...
            <div className="text-sm text-gray-700 whitespace-pre-line leading-relaxed">
              {activeMaterial.content || "No detailed notes provided for this lesson."}
            </div>
          </div>
        </div>
      )}
